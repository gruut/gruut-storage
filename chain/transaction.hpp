...
public:
  Transaction() : m_sent_time(0), m_transaction_id({0x00}), m_transaction_type(TransactionType::UNKNOWN) {}

  Transaction(tx_id_type transaction_id, timestamp_t sent_time, requestor_id_type &requestor_id, TransactionType transaction_type,
              base64_type &signature, std::vector<content_type> &content_list)
      : m_transaction_id(transaction_id), m_sent_time(sent_time), m_requestor_id(requestor_id), m_transaction_type(transaction_type),
  ...
  bool setJson(json &tx_json) {
    ...
    setId(TypeConverter::bytesToArray<TRANSACTION_ID_TYPE_SIZE>(new_txid_bytes));
  ...
  template <typename T = tx_id_type>
  void setId(T &&transaction_id) {
  ...
  void setTime(timestamp_t sent_time) {
  ...
  template <typename T = base64_type>
  void setSignature(T &&signature) {
    m_signature = signature;
  }

  template <typename T = json>
  void setContentsFromJson(T &&content_list) {
...
private:
  ...
  tx_id_type buildTxId() {
    ...
    return static_cast<tx_id_type>(TypeConverter::bytesToArray<TRANSACTION_ID_TYPE_SIZE>(Sha256::hash(txid_builder.getBytes())));
  }
};
