...
public:
  ...
  bool operator==(Block &other) const {
    return (m_block_height == other.getHeight() && m_block_hash == other.getBlockHashB64());
  }
  ...
  bool initialize(json &msg_block) {
    ...
    m_world_id = TypeConverter::base64ToArray<CHAIN_ID_TYPE_SIZE>(Safe::getString(msg_block["block"], "world"));
    m_chain_id = TypeConverter::base64ToArray<CHAIN_ID_TYPE_SIZE>(Safe::getString(msg_block["block"], "chain"));
    ...
    m_sg_root = Safe::getString(msg_block["state"], "sgroot");
    ; // 추후 계산, 검증 필요
